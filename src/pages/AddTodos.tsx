...
import { Text, View, TextInput, StyleSheet, Switch, TouchableOpacity } from 'react-native'
      ...
      <View style={{ backgroundColor: theme.colors.border, ...styles.TaskContainer }}>
        <View style={styles.TitleContainer}>
          <TextInput ref={titleRef} style={[{ color: theme.colors.primary, borderColor: theme.colors.primary, ...styles.TaskTitle }, editTitle ? { 'borderBottomWidth': 1 } : { 'borderBottomWidth': 0 }]}
            editable={editTitle} onBlur={() => { setEditTitle(false) }} placeholderTextColor={theme.colors.primary} placeholder="Task Title" onChangeText={(text) => { clearTimeout(timeout); timeout=setTimeout(()=>{setTitle(text)},500) }} />
          <TouchableOpacity onPress={() => { setEditTitle(true) }}><Edit height={35} width={35} color={theme.colors.primary} /></TouchableOpacity>
        </View>
        <TextInput style={{ color: theme.colors.primary, borderColor: theme.colors.primary, ...styles.TextLine }} placeholderTextColor={theme.colors.primary} placeholder="Task Description" onChangeText={(text) => { clearTimeout(timeout); timeout=setTimeout(()=>{setDesc(text)},500) }}  />
        <View>
        ...
        <TextInput style={{ color: theme.colors.primary, borderColor: theme.colors.primary, ...styles.TextLine }} placeholderTextColor={theme.colors.primary} placeholder="Points for Task" keyboardType="numeric"  onChangeText={(text) => { clearTimeout(timeout); timeout=setTimeout(()=>{setPoints(+text)},500) }} />
